...
trait LeagueReport
{
    public function getLeagueReport()
    {
        ...
        foreach ($teams as $team) {
            ...
            $team_home_matches = DB::table('matches')
                ->whereIn('league_week_id', $team_home_league_weeks->pluck('id'))
                ->get();
            $team_away_matches = DB::table('matches')
                ->whereIn('league_week_id', $team_away_league_weeks->pluck('id'))
                ->get();
    ...
    public function getChampionPredict($teams_state)
    {
        ...
        foreach ($teams_state as $team_state) {
            ...
            $temp->prediction = $team_state->pts ?
                intval($team_state->pts * $team_state->strength / $total_strength * 100) : 0;
